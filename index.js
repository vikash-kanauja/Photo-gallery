...
const openPopup = (index) => {
  ...
  popupModal.addEventListener("keydown", closePopup);
};

const closePopup = (event) => {
  if (event.target === popupModal || event.key === "Escape") {
...
let numberOfImage = imagesPerRowSelect.value;
imagesPerRowSelect.addEventListener("change", () => {
    numberOfImage = imagesPerRowSelect.value;
  setGrid(imagesPerRowSelect.value);
});

const setGrid = (numberOfImage) => {
  gallery.style.gridTemplateColumns = `repeat(${numberOfImage}, minmax(0, 1fr))`;
};
setGrid(numberOfImage);
